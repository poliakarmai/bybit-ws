...
|---------|----------|-----------|
| Закрытых сделок | 228 | (всего с открытыми) |
| Win Rate (все) | 60% | |
| Profit Factor (все) | 0.82 | исторический (до Фазы 9.1 был 0.59) |
| **PF после Фазы 9.1 (15.08+)** | **2.36** | ✅ WR 74% — junk-фикс закрыл дисбаланс |
| SHORT после 9.1 | WR 83%, PF 4.50 | avg_loss $12.59 → $6.94 |
| Режим | TRENDING_DOWN | LSTM 82.3% точность |

> **Risk/reward — РЕШЁН Фазой 9.1 (15.08.2026).** До 9.1 junk-шорты (Tier C/D) входили без SL + DCA-мартингейл → SHORT avg_loss $12.59 vs avg_win $4.71 (ratio 2.68×). Фаза 9.1 (SL +7% через trading-stop, DCA-лесенка удалена) убрала хвостовые убытки: после 15.08 SHORT ratio 1.05×, PF 4.50, общий PF 2.36. Исторический PF 0.82 тянет вниз только старый период (до 15.08).
